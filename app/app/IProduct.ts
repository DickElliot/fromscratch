...
export class Product implements IProduct {
  ...
  constructor(product: IProduct) {
    ...
    this.purchaseUnit = createUnitType(product.unit);
  ...
  private createSupermarketSection(section: string): string {
    switch (section) {
      ...
      case ('pantry'):
        return 'Pantry';
      default:
        console.log(`No supermarket section found for ${section}`);
        return 'Unknown';
    }
    // return 1;
  }
...
export class PricedProduct extends Product implements IPricedProduct {
  ...
  // 3) Allow setting of product amount & price painless & more in line with OO practices, e.g. no class-external meddling
  ...
  /*   private setPurchasePrice() { }
    set setPurchaseAmount(amount: amount) {
        let purchaseAmount = 0;
        const whole: { [key: string]: number } = {
            small: 1, medium: 1.5, large: 1.7, item: 1, whole: 1, canned: 1,
        }
        const variable: { [key: string]: number } = {
            ounce: 1, g: 1, kg: .01,
        }
        const approxWeights: { [key: string]: number } = {
            pumpkin: 1200,
            squash: 1000,
            'garlic cloves': 10,
            onion: 100
        }
        if (this.unitType == unitType.fixed) {
            if (variable[unit] !== undefined) {
                purchaseAmount = quantity * variable[this.unit];
            }
            else if (whole[unit] !== undefined) {
                purchaseAmount = quantity * whole[this.unit];
            }
 
        } else if (this.unitType == unitType.variable) {
            if (unit == 'kg') {
