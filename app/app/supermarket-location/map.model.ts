        ...
        this.map.on('click', (evt) => {
            for (let featureName of this.featureNames) {
                if (this.map.forEachFeatureAtPixel(evt.pixel, (feature) => {
                    ...
                    // console.log(featureName + ' chosen!');
                }
            }
        });
    }
    ...
    addSupermarketLocationToMap(supermarketName: string, longitude: number, latitude: number) {
        ...
        let labelStyle = new Style({
            ...
            image: new Icon({
                anchor: [0.5, 0.5],
                offset: [52, 0],
                // anchorXUnits: fractionUnits,
                // anchorYUnits: pixelUnits,
                src: 'http://localhost/fromscratch/images/countdown_mini.jpg',
                opacity: 1,
                scale: 0.25,
                size: [52, 52],
            })
        });
        let fractionUnits: any = 'fraction';
        let pixelUnits: any = 'pixel';
